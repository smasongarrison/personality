# (PART) Module 02 {-}

```{r include = FALSE}
source("common.R")

# install.packages("devtools")

if (!require("tweetrmd"))  devtools::install_github("gadenbuie/tweetrmd")
library(tweetrmd) #... embedding tweets
class_urls <- read.csv("./data/class_urls.csv")
```

```{r links, child="links.md"}
```

# Welcome to Personality Methods

This module is designed to introduce you to methods of personality research. 
Please watch the videos and work your way through the notes. 
**The videos start on the next page.** 
You can find the module playlist [here][pl-02].

## Learning Goals

By the end of this module, you should be able to achieve all the learning goals outlined below:

* Understand that personality is not directly observable, and data are clues.
* Recognize and classify instances of B, L, I, and S data.
* Define and distinguish B, L, I, and S data.
* Identify strengths and weaknesses of B, L, I, and S data.
* Understand new concepts and terminology associated with different types of data.
* Differentiate the goals of scientific training from technical training.
* Define and distinguish reliability and validity.
* Give examples of techniques researchers can use to improve measurement quality.
* Identify a variety of issues related to generalizability.
* Compare and contrast different types of research designs.


## Module Materials

### Explore
* Readings
	* Chapter 2 from the Personality Puzzle
		  *  [Norton Ebook Link][[ppch2_method]]
* Videos
  * Located in the subchapters of this module
* Slidedecks
	* Located on Canvas

### Engage
* Activities
	* Located on Canvas

### Evaluate
* Quiz
	* Located on Canvas


# Lecture: Personality Research Methods {#methodsvideo}

This sequence of videos covers personality research methods. It is approximately 2 hours in length using this [neat webapp][ytplaylist]. Specifically,

* No of videos : 10

* Average length of video : 11 minutes, 49 seconds

* Total length of playlist : 1 hour, 58 minutes, 14 seconds

## Clues to Personality (02.01)
<!-- updated -->

```{r, echo=FALSE}
"https://www.youtube.com/watch?v=XN1GeS7FSio" %>%
 embed_url() %>%
  use_align("center")
```

## Self Data (02.02.01)
<!-- updated; original was two parts; this notation is here because the numbering differs across videos -->

```{r, echo=FALSE}
"https://www.youtube.com/watch?v=1Bdt3v6bW2k" %>%
 embed_url() %>%
  use_align("center")
```

## Informant Data (02.02.02)
<!-- updated -->

```{r, echo=FALSE}
"https://www.youtube.com/watch?v=J9bMPFLlpAc" %>%
 embed_url() %>%
  use_align("center")
```

## Life Outcome and Behavioral Data (02.03)

<!-- updated -->

```{r, echo=FALSE}
"https://www.youtube.com/watch?v=TSbMg14CqRQ" %>%
 embed_url() %>%
  use_align("center")
```

## Data Quality (02.04)
<!-- keep original -->
```{r, echo=FALSE}
"https://www.youtube.com/watch?v=Py-3LjJYzKk" %>%
 embed_url() %>%
  use_align("center")
```

## Introducing Validity and What's a construct? (02.05)
<!-- keep original -->

```{r, echo=FALSE}
"https://youtu.be/MgTZUP7Dzlk" %>%
 embed_url() %>%
  use_align("center")
```

## Generalizability (02.06)

<!-- updated  -->
```{r, echo=FALSE}
"https://youtu.be/xpPlRDn4WJM" %>%
 embed_url() %>%
  use_align("center")
```

## Causal Inference and Generalizability in Action (02.07)

<!-- updated  -->
```{r, echo=FALSE}
"https://youtu.be/lLk8iWtg22A" %>%
 embed_url() %>%
  use_align("center")
```

## Research Designs (02.08)
<!-- updated-->

```{r, echo=FALSE}
"https://www.youtube.com/watch?v=6MKSjsPfYhc" %>%
 embed_url() %>%
  use_align("center")
```

## Correlational Methods (02.09)

<!-- updated-->
```{r, echo=FALSE}
"https://youtu.be/E1e5sHbcVfE" %>%
 embed_url() %>%
  use_align("center")
```
